...
1. **Free Tier RPM 제한에 따른 속도 한계**
   - 대용량 문서(100페이지 이상) 분석 시 지수 백오프 대기 시간으로 인해 최초 처리 시간이 다소 소요됨.
   - *향후 과제*: Celery/Redis 기반 비동기 처리 도입 및 유료 API 플랜 전환 옵션 추가.

2. **복잡한 표/이미지 데이터 처리의 한계**
